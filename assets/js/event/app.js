    ...
    methods: {
      ...
      doesTransparentTotalDiverge(input) {
        ...
        if (Array.isArray(input)) {
          ...
          // Si le total transparent a plus d'une entrée (ex: "1 Kg + 200 gr."), il y a forcément divergence.
          if (details.some(detail => {
            const qTotalX = detail.qTotalX || [];
            return qTotalX.length > 1;
          })) {
            return true;
          }
        } else {
        ...
        // On cherche si au moins UN des ingrédients de la liste a le drapeau "hasConversion"
        // OU si les unités standardisées diffèrent des unités originales
        const hasAnyConversion = details.some(detail => {
          // Vérifier si le drapeau hasConversion est présent
          if (detail.hasConversion === true) {
            return true;
          }

          // Vérifier si les unités standardisées diffèrent des unités originales
          // if (detail.quantiteEq && detail.unitEq) {
          //   const originalUnit = (detail.unit || '').toLowerCase();
          //   const standardUnit = (detail.unitEq || '').toLowerCase();

          //   // Comparer directement les unités (elles sont déjà normalisées)
          //   return originalUnit !== standardUnit;
          // }
      ...
      // convertUnit(item) {
      //   if (item && (item.unit === 'gr.' || item.unit === 'gr.')) {
      ...
      round2Decimals(x, unit = '') {
        ...
        if (unitLower === 'kg' || unitLower === 'l.' || unitLower === 'l') {
          ...
          return Math.round(num); // Arrondi à l'unité pour les autres
        }
      },
