 ...
 *
 * @services
 * - DataTransformer: ./services/DataTransformer.js - Formatage des données pour l'UI
 * - SyncService: ./services/SyncService.js - Synchronisation différentielle cache ↔ Appwrite
 * - localStorageService: ./services/localStorageService.js - Gestion du cache local (données + timestamp)
 * - AppwriteDataService: ../AppwriteDataService.js - CRUD Appwrite (create/update/delete)
 * - UnitsManager: ./UnitsManager.js - Gestion des conversions d'unités
 * - ColorManager: ./ColorManager.js - Attribution de couleurs pastel (volontaires/magasins)
 * - TableColumnsConfig: ../TableColumnsConfig.js - Configuration des colonnes TanStack Table
 * - ModalMixin: ../ModalMixin.js - Gestion unifiée des modaux
 * - AuthManager: ./services/AuthManager.js - Gestion authentification Appwrite
 *
 * @data-flow
  ...
  * ```mermaid
   graph LR
       subgraph "Services Layer"
           AM[AuthManager<br/>./services/AuthManager.js]
           LS[localStorageService<br/>./services/localStorageService.js]
           SS[SyncService<br/>./services/SyncService.js]
           DT[DataTransformer<br/>./services/DataTransformer.js]
           IC[IngredientCalculator<br/>../event/ingredientCalculator.js]
           AD[AppwriteDataService<br/>../AppwriteDataService.js]
           UM[UnitsManager<br/>./UnitsManager.js]
           CM[ColorManager<br/>./ColorManager.js]
           TC[TableColumnsConfig<br/>../TableColumnsConfig.js]
           MM[ModalMixin<br/>../ModalMixin.js]
       end
       ...
       subgraph "Vue Application"
           VA[app_aw_ingredients.js<br/>Main Vue Component]
           UI[Vue Components<br/>TanStack Table/Modals]
       end
       ...
       classDef vue fill:#e8f5e8

       class AM,LS,SS,DT,IC,AD,UM,CM,TC,MM service
       class AW,RT,LS2 external
       class VA,UI vue
  * ```
 *
 */
...
import {
  ...
  getFilteredRowModel,
  getExpandedRowModel,
} from "js/vuetable.development.js";
...
export function createCollaborativeApp() {
  const app = Vue.createApp({
    ...
    computed: {
      ...
      showAuthModal() {
        return (
          !this.isAuthenticated && this.authManager !== null && !this.isLoading
        );
      },
      ...
      filteredIngredients() {
        const filtered = this.transformedIngredients.filter((ingredient) => {
          // Filtre par recherche (inclut les noms précédents)
          if (this.searchQuery.trim()) {
            const query = this.searchQuery.toLowerCase().trim();
            const nameMatches = ingredient.ingredientName
              .toLowerCase()
              .includes(query);

            // Vérifier les noms précédents
            const previousNamesMatch =
              ingredient.previousNames &&
          ...
          if (this.selectedStoreFilter) {
            if (
              !ingredient.store ||
              !ingredient.store.includes(this.selectedStoreFilter)
            ) {
              return false;
            }
          }
          ...
          if (this.selectedPersonFilter) {
            if (
              !ingredient.who ||
              !ingredient.who.includes(this.selectedPersonFilter)
            ) {
              return false;
            }
          }
      ...
      tableData() {
        const data = this.filteredIngredients.map((ing) => ({
          ...
          responsibleDisplay:
            ing.who && ing.who.length > 0 ? ing.who.join(", ") : "",
        }));
      ...
      isPurchaseFormValid() {
        return (
          this.newPurchase && this.newPurchase.quantity && this.newPurchase.unit
        );
      },
      ...
      selectedCount() {
        return this.modalSelectedIngredients.filter((ing) => ing.selected)
          .length;
      },
      ...
      tableColumns() {
        ...
        const config = new TableColumnsConfig({
          ...
          handlers: {
            ...
            handleRenameIngredient: this.handleRenameIngredient
          },
          formatters: {
          ...
          onRowSelectionChange: (updater) => {
            const newSelection =
              typeof updater === "function"
                ? updater(this.table?.getState().rowSelection)
                : updater;
            ...
            console.log(
              "[Collaborative App] Selected ingredient IDs:",
              selectedIds,
            );
    ...
    methods: {
      ...
      async checkAuthentication() {
        ...
        try {
          ...
          console.error(
            "[Collaborative App] Erreur lors de la vérification de l'authentification:",
            error,
          );
      ...
      async initializeApp() {
        try {
          ...
          if (this.isAuthenticated) {
            ...
            console.log(
              "[Collaborative App] Synchronisation temps réel désactivée (utilisateur non authentifié)",
            );
          }
        ...
        try {
          ...
          if (cacheResult.success && cacheResult.fromCache) {
            ...
            this.syncChangesInBackground().catch((error) => {
              console.error(
                "[Collaborative App] Erreur de synchronisation en arrière-plan:",
                error,
              );
            });
          } else {
            ...
            console.log(
              "[Collaborative App] Première visite, chargement depuis Appwrite...",
            );
            await this.loadInitialData();
          }
        } catch (error) {
          console.error(
            "[Collaborative App] Erreur lors du chargement avec cache:",
            error,
          );
        ...
        if (syncResult.success) {
          ...
          if (hasRealChanges) {
            ...
            if (isFirstVisit) {
              ...
              changes.ingredients.length > 5 ||
              changes.purchases.length > 3
            ) {
              // Beaucoup de changements : utiliser la mise à jour optimisée
              this.updateMultipleFromSyncChanges(
                changes.ingredients,
                changes.purchases,
              );
            } else {
            ...
            console.log(
              "[Collaborative App] Données mises à jour après synchronisation",
            );
          } else {
            console.log(
              "[Collaborative App] Aucun changement à synchroniser, pas de retraitement",
            );
          }
        }
      },
      ...
      async loadInitialData() {
        console.log(
          "[Collaborative App] Chargement des données initiales depuis Appwrite...",
        );
        try {
      ...
      async transformDataForUI(useOptimizedUpdate = false) {
        console.log(
          "[Collaborative App - transformDataForUI] Transformation des données pour l'UI...",
          {
        ...
        try {
          ...
          // Utiliser la nouvelle méthode centralisée qui intègre tous les traitements
          this.transformedIngredients = DataTransformer.transformAllForUI(
            this.ingredients,
            this.purchases,
          );

          console.log(
            "[Collaborative App] Transformation terminée -",
      ...
      updateIngredientComplete(ingredientPayload, eventType) {
        console.log(
          "[Collaborative App] Mise à jour complète ingrédient:",
          ingredientPayload.$id,
          eventType,
        );

        // 1. Mettre à jour les données brutes
        this._updateLocalCollection(
          this.ingredients,
          ingredientPayload,
          eventType,
        );
        ...
        if (eventType !== "delete") {
          ...
          const index = this.transformedIngredients.findIndex(
            (ing) => ing.$id === ingredientPayload.$id,
          );
          if (index !== -1) {
      ...
      updatePurchaseComplete(purchasePayload, eventType) {
        console.log(
          "[Collaborative App] Mise à jour complète achat:",
          purchasePayload.$id,
          eventType,
        );
        ...
        if (purchasePayload.listIngredient) {
          ...
          if (
            typeof purchasePayload.listIngredient === "object" &&
            purchasePayload.listIngredient.$id
          ) {
        ...
        if (affectedIngredientId) {
          ...
          console.warn(
            "[Collaborative App] Impossible de déterminer l'ingrédient lié pour l'achat, mise à jour UI ignorée:",
            purchasePayload.$id,
          );
        }
      },
      ...
      updateMultipleFromSyncChanges(ingredientChanges, purchaseChanges) {
        ...
        try {
          ...
          console.error(
            "[Collaborative App] Erreur lors de la mise à jour multiple syncChanges:",
            error,
          );
      ...
      updateSingleIngredientInUI(ingredientIdentifier) {
        ...
        const ingredientId =
          typeof ingredientIdentifier === "object" &&
        ...
        if (!ingredientId || typeof ingredientId !== "string") {
          console.error(
            "[Collaborative App] ID d'ingrédient invalide pour la mise à jour UI:",
            ingredientIdentifier,
          );
          return;
        }
        ...
        if (!ingredient) {
          console.warn(
            "[Collaborative App] Ingrédient non trouvé pour mise à jour:",
            ingredientId,
          );
          return;
        }
        ...
        try {
          ...
          const transformedIngredient =
            DataTransformer.createCompleteIngredient(
              ingredient,
              this.purchases || [],
            );

          if (!transformedIngredient) return;

          // Mettre à jour toutes les occurrences de cet ingrédient dans transformedIngredients
          this._updateAllIngredientOccurrences(ingredientId, [
            transformedIngredient,
          ]);

          // Mettre à jour l'ingrédient dans les données brutes
          ...
          const store = transformedIngredient.store;
          if (
            store &&
            store !== "-" &&
            !this.syncService.storesCache.includes(store)
          ) {
            this.syncService.storesCache.push(store);
            this.syncService.storesCache.sort();
          }

          (transformedIngredient.who || []).forEach((person) => {
          ...
          console.error(
            "[Collaborative App] Erreur lors de la mise à jour optimisée:",
            error,
          );
      ...
      _updateAllIngredientOccurrences(ingredientId, newTransformedIngredients) {
        ...
        if (indicesToUpdate.length === newTransformedIngredients.length) {
          ...
          indicesToUpdate.forEach((index, i) => {
            this.transformedIngredients.splice(
              index,
              1,
              newTransformedIngredients[i],
            );
          });
        } else {
      ...
      updateMultiplePurchasesInUI(purchaseIds) {
        ...
        try {
          ...
          if (purchasesToUpdate.length === 0) {
            console.warn(
              "[Collaborative App] Aucun achat trouvé pour mise à jour:",
              purchaseIds,
            );
            return;
          }
          ...
          console.log(
            "[Collaborative App] Achats mis à jour de manière optimisée:",
            purchasesToUpdate.length,
            "→",
            uniqueAffectedIds.length,
            "ingrédients affectés",
          );
        } catch (error) {
          console.error(
            "[Collaborative App] Erreur lors de la mise à jour optimisée multiple achats:",
            error,
          );
          // Fallback : retransformer toutes les données
          this.transformDataForUI();
      ...
      updateMultipleIngredientsInUI(ingredientIds) {
        ...
        try {
          // Trouver les ingrédients bruts à mettre à jour
          const ingredientsToUpdate = this.ingredients.filter((ing) =>
            ingredientIds.includes(ing.$id),
          );

          // Utiliser la nouvelle méthode centralisée pour transformer tous ces ingrédients
          const transformedIngredients = DataTransformer.transformAllForUI(
            ingredientsToUpdate,
            this.purchases || [],
          );
          ...
          const updatedIds = new Set(
            transformedIngredients.map((ing) => ing.$id),
          );
          const unchangedIngredients = this.transformedIngredients.filter(
            (ing) => !updatedIds.has(ing.$id),
          );

          // Fusionner les ingrédients inchangés avec les ingrédients mis à jour
          this.transformedIngredients = [
            ...unchangedIngredients,
            ...transformedIngredients,
          ];

          console.log(
            "[Collaborative App] Ingrédients mis à jour de manière optimisée:",
            ingredientIds.length,
          );
        } catch (error) {
          console.error(
            "[Collaborative App] Erreur lors de la mise à jour optimisée multiple:",
            error,
          );
          // Fallback : retransformer toutes les données
          this.transformDataForUI();
      ...
      updateIngredientsFromPurchasesInUI(updatedPurchases) {
        ...
        try {
          ...
          const affectedIngredientIds = new Set(
            updatedPurchases.map((p) => p.listIngredient),
          );

          // 2. Trouver les ingrédients bruts qui doivent être mis à jour
          const ingredientsToRecalculate = [];
          ...
          if (ingredientsToRecalculate.length === 0) return;

          // 3. Utiliser la nouvelle méthode centralisée pour transformer ces ingrédients
          const transformedIngredients = DataTransformer.transformAllForUI(
            ingredientsToRecalculate,
            this.purchases || [],
          );

          // 4. Mettre à jour les ingrédients dans le tableau transformé
          transformedIngredients.forEach((updatedIngredient) => {
            const index = this.transformedIngredients.findIndex(
              (ing) => ing.$id === updatedIngredient.$id,
            );
            if (index !== -1) {
              this.transformedIngredients[index] = updatedIngredient;
            }
          });

          console.log(
            `[Collaborative App] ${transformedIngredients.length} ingrédient(s) mis à jour suite à un changement d'achat.`,
          );
        } catch (error) {
          console.error(
            "[Collaborative App] Erreur lors de la mise à jour depuis achats:",
            error,
          );
      ...
      setupRealtime() {
        ...
        try {
          ...
          console.error(
            "[Collaborative App] Erreur lors de la configuration realtime:",
            error,
          );
      ...
      scheduleReconnection() {
        ...
        const delay =
          this.realtimeStatus.retryDelay * (this.realtimeStatus.retryCount + 1);
      ...
      handleRealtimeUpdate(response) {
        ...
        if (this.syncService) {
          ...
          if (!syncResult) {
            console.warn(
              "[Collaborative App] Le changement n'a pas pu être appliqué au cache",
            );
          }
        }
        ...
        if (collectionName === APPWRITE_CONFIG.collections.ingredients) {
          ...
          if (this.localStorageService) {
            ...
            const transformedIngredient = this.transformedIngredients.find(
              (ing) => ing.$id === payload.$id,
            );
            if (transformedIngredient) {
              this.localStorageService.updateFromIngredient(
                transformedIngredient,
              );
      ...
      setView(viewName) {
        ...
        try {
          ...
          console.warn(
            "Impossible de sauvegarder la préférence de vue:",
            error,
          );
        }
      },
      ...
      loadPreferredView() {
        try {
          ...
          if (
            savedView &&
            ["table", "grouped", "cards", "compact"].includes(savedView)
          ) {
      ...
      openGroupAssignmentModalForSelection(type) {
        try {
          ...
          const selectedIngredientObjects = this.selectedIngredients
            .map((id) =>
          ...
          if (type === "volunteer") {
            this.openGroupWhoModal(
              context,
              groupName,
              selectedIngredientObjects,
            );
          } else if (type === "store") {
            this.openGroupStoreModal(
              context,
              groupName,
              selectedIngredientObjects,
            );
          ...
          console.error(
            "[Collaborative App] Erreur lors de l'ouverture du modal de sélection:",
            error,
          );
      ...
      isAssignmentToRemove(ingredientId, value) {
        return (
          this.groupAssignment.assignmentsToRemove
            .get(ingredientId)
            ?.has(value) || false
        );
      },
      ...
      toggleAssignmentRemoval(ingredientId, value) {
        ...
        const removals =
          this.groupAssignment.assignmentsToRemove.get(ingredientId);
      ...
      getActionType() {
        ...
        const hasManualRemovals = Array.from(
          this.groupAssignment.assignmentsToRemove.values(),
      ...
      markAllAssignmentsForRemoval() {
        ...
        this.modalSelectedIngredients.forEach((ingredient) => {
          ...
          const existingAssignments =
            this.groupAssignment.type === "volunteer"
              ? ingredient.who || []
              : ingredient.store || [];

          if (existingAssignments.length > 0) {
            this.groupAssignment.assignmentsToRemove.set(
              ingredient.$id,
        ...
        this.showToast(
          "Toutes les attributions existantes seront remplacées",
          "warning",
          3000,
          "Mode remplacement",
        );
      ...
      updateSummary() {
        ...
        const selectedIngredients = this.modalSelectedIngredients.filter(
          (ing) => ing.selected,
        );

        for (const ingredient of selectedIngredients) {
          const hasRemovals = this.groupAssignment.assignmentsToRemove.has(
            ingredient.$id,
          );
          ...
          if (this.groupAssignment.type === "store") {
            ...
            if (hasRemovals) {
              removalsCount += this.groupAssignment.assignmentsToRemove.get(
                ingredient.$id,
              ).size;
            }
      ...
      exportSelectedIngredients() {
        try {
          ...
          link.setAttribute(
            "download",
            `ingredients_selection_${new Date().toISOString().split("T")[0]}.csv`,
          );
          ...
          this.showSuccessToast(
            `${selectedIngredients.length} ingrédients exportés`,
          );
        } catch (error) {
          console.error(
            "[Collaborative App] Erreur lors de l'export des ingrédients:",
            error,
          );
      ...
      deselectAllMainIngredients() {
        if (
          !this.table ||
          typeof this.table.getToggleAllRowsSelectedHandler !== "function"
        ) {
          console.warn(
            "[Collaborative App] Table non disponible pour la désélection",
          );
          return;
        }
        ...
        try {
          ...
          console.error(
            "[Collaborative App] Erreur lors de la désélection de tous les éléments:",
            error,
          );
        }
      },
      ...
      handleOpenMergeModal() {
        ...
        if (selectedIngredients.length < 2) {
          this.showErrorToast(
            "Veuillez sélectionner au moins 2 ingrédients à fusionner",
          );
          return;
        }
      ...
      async processGroupAssignment(ingredients) {
        console.log(
          "[Collaborative App] Traitement groupé pour",
          ingredients.length,
          "ingrédients",
        );
        const updates = [];
        ...
        for (const ingredient of ingredients) {
          let patchData = {};
          if (this.groupAssignment.type === "volunteer") {
            const currentVolunteers = Array.isArray(ingredient.who)
              ? ingredient.who
              : [];
            const newVolunteer = this.groupAssignment.value.trim();
            const volunteersToRemove =
              this.groupAssignment.assignmentsToRemove.get(ingredient.$id) ||
              new Set();
            let finalVolunteers = currentVolunteers.filter(
              (v) => !volunteersToRemove.has(v),
            );
        ...
        if (updates.length > 0) {
          await this.appwriteDataService.batchUpdateIngredients(
            updates,
            this.database,
          );
        } else {
          console.warn(
            "[Collaborative App] Aucune mise à jour à effectuer pour l'attribution groupée.",
          );
        }
      },
      ...
      async handleGroupAssignmentSubmit() {
        ...
        try {
          ...
          const typeName =
            this.groupAssignment.type === "volunteer" ? "Bénévole" : "Magasin";
          this.showSuccessToast(
            `${typeName} assigné avec succès à ${selectedIngredients.length} ingrédient(s).`,
          );
          ...
          console.error(
            "[Collaborative App] Erreur lors de l'attribution groupée:",
            error,
          );
      ...
      hasStoreModifications() {
        const hasValue = this.groupAssignment.value.trim();
        const hasSelectedIngredients = this.modalSelectedIngredients.some(
          (ing) => ing.selected,
        );
      ...
      async handleAppwriteError(error, context) {
        ...
        if (error.code === 409) {
          userMessage =
            "Conflit de données: Veuillez rafraîchir la page et réessayer.";
    ...
    beforeUnmount() {
      ...
      if (this.unsubscribeRealtime) {
        console.log(
          "[Collaborative App] Désabonnement des mises à jour temps réel.",
        );
        this.unsubscribeRealtime();
      }
    },
    ...
    async forceFullSync() {
      if (this.syncService) {
        ...
        try {
          ...
          await this.loadInitialData();
          console.log(
            "[Collaborative App] Synchronisation complète forcée terminée",
          );
        } catch (error) {
          console.error(
            "[Collaborative App] Erreur lors de la synchronisation forcée:",
            error,
          );
        } finally {
          this.isLoading = false;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (appRoot) {
    ...
    console.error(
      "[Collaborative App] L'élément racine #collaborativeApp est introuvable. L'application ne peut pas démarrer.",
    );
  }
});
